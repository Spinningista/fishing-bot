...
def search_waters(conn, query: str, limit: int = 10) -> list[sqlite3.Row]:
    q = query.lower()
    rows = conn.execute("SELECT * FROM waters ORDER BY name").fetchall()
    matches = [r for r in rows if q in r["name"].lower()]
    return matches[:limit]


def water_ref_count(conn, water_id: int) -> int:
    return conn.execute("SELECT COUNT(*) as c FROM trips WHERE water_id = ?", (water_id,)).fetchone()["c"]
...
def search_lures(conn, query: str, limit: int = 10) -> list[sqlite3.Row]:
    # Фильтруем на стороне Python, а не через SQL lower() — у SQLite нет
    # встроенной поддержки приведения регистра для кириллицы, поиск по
    # русским названиям иначе просто ничего не находил.
    q = query.lower()
    rows = conn.execute("SELECT * FROM lures ORDER BY brand, model").fetchall()
    matches = [r for r in rows if q in f"{r['brand']} {r['model']}".lower()]
    return matches[:limit]
...
def lure_ref_count(conn, lure_id: int) -> int:
    return conn.execute("SELECT COUNT(*) as c FROM catches WHERE lure_id = ?", (lure_id,)).fetchone()["c"]


def delete_lure(conn, lure_id: int):
    conn.execute("DELETE FROM lures WHERE id = ?", (lure_id,))
...
def search_species(conn, query: str, limit: int = 10) -> list[sqlite3.Row]:
    q = query.lower()
    rows = conn.execute("SELECT * FROM species ORDER BY name").fetchall()
    matches = [r for r in rows if q in r["name"].lower()]
    return matches[:limit]


def species_ref_count(conn, species_id: int) -> int:
    return conn.execute("SELECT COUNT(*) as c FROM catches WHERE species_id = ?", (species_id,)).fetchone()["c"]


def delete_species(conn, species_id: int):
